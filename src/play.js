...
const ALL_SETS = {
  ...
  potion: { levels: POTION_LEVELS, key: "magmamia.solved.potion.v1", title: "With a Little Help", blurb: "Seize the crown wearing the robe of protection: one safe step onto magma, then it's used up." },
...
const COMMENTARY_KEY = "magmamia.commentary.v1";
...
const state = {
  ...
  commentary: true, // Show the per-move message (picked up/used the robe, refusals, deaths). On by default, opt-out.
  hasPotions: false, // Whether the current level uses potions at all, set by startLevel.
  startsProtected: false // Whether the level hands the player a charge up front, set by startLevel.
};
...
const elements = {
  ...
  getHelpButton: document.querySelector("#get-help-button"),
  ...
  commentary: document.querySelector("#show-commentary"),
  board: document.querySelector("#board"),
  boardStage: document.querySelector("#board-stage"),
  solvedBadge: document.querySelector("#solved-badge"),
  status: document.querySelector("#status"),
  undo: document.querySelector("#undo-button"),
  restart: document.querySelector("#restart-button"),
  next: document.querySelector("#next-button"),
  objectiveHelp: document.querySelector("#objective-help"),
  robeHelp: document.querySelector("#robe-help"),
...
elements.commentary.checked = state.commentary;
...
elements.commentary.addEventListener("change", () => {
  state.commentary = elements.commentary.checked;
  savePreference(COMMENTARY_KEY, state.commentary ? "1" : "0");
  render();
});
...
// The splash's second option: same idea as "Enter the dungeon!", but for the
// "With a little help" set instead of the implicit "crown" one -- has to switch
// setKey and reload that set's own solved progress first, which showSplash()
// never does for anything but crown.
elements.getHelpButton.addEventListener("click", () => {
  state.setKey = "potion";
  loadSetPreferences();
...
// --- Preferences: three global display settings, plus each set's own solved list --
...
function loadPreferences() {
  try {
    ...
    state.commentary = window.localStorage.getItem(COMMENTARY_KEY) !== "0"; // On unless turned off.
  } catch {
...
function startLevel(index) {
  ...
  // A level can hand the player a charge up front (see POTION_LEVELS in
  // levels.js) instead of -- or as well as -- an on-board `!` to walk onto;
  // either way counts as "uses potions" for showing the carried count below.
  state.startsProtected = Boolean(levels[index].carried);
  if (state.startsProtected) state.initial = { ...state.initial, carried: levels[index].carried };
  state.hasPotions = state.startsProtected || state.initial.potions.some((p) => p === 1);
  resetHistory();
  setMessage("", "");
  render();
}
...
function attemptMove(directionName) {
  ...
  if (outcome.result === "moved" || outcome.result === "pushed") {
    ...
    else if (outcome.usedPotion) setMessage("The robe saved you -- but it's used up now, so don't step onto magma again next.", "");
    else setMessage("", "");
    if (isSolved()) {
...
function handleKeyDown(event) {
  ...
  // Checkboxes are the exception: the settings ones stay focused after a click,
  // and skipping them would leave the arrow keys dead until you click elsewhere.
  if ((tag === "INPUT" && event.target.type !== "checkbox") || tag === "SELECT" || tag === "TEXTAREA") return;
  ...
  if (directionName) {
    ...
    // Without this, Enter also "clicks" whichever button still has focus (the i
    // button, if you'd just closed the info panel) as well as advancing.
    event.preventDefault();
...
// The "solved" badge (see .solved-badge in styles.css): the player wearing the
// crown, faded and scaled in on the same delayed beat as scheduleMelt above,
// and sharing its timer map -- distinct elements never collide as WeakMap keys.
// The board fades out completely at the same moment (.board-stage.is-celebrating,
// not just its walls) and the stage takes on a fixed height, so the badge sits
// in the same size, plain area regardless of the level's own board size --
// no trace of the level's floor colour or shape behind the badge.
function scheduleBadgeReveal(stage, badge, shown) {
  const pending = meltTimers.get(badge);
  if (!shown) {
    if (pending) clearTimeout(pending);
    meltTimers.delete(badge);
    badge.classList.remove("is-shown");
    stage.classList.remove("is-celebrating");
    return;
  }
  if (pending || badge.classList.contains("is-shown")) return;
  meltTimers.set(badge, setTimeout(() => {
...
function renderPlay() {
  elements.title.textContent = "Magma Mia!";
  ...
  elements.robeHelp.hidden = !state.startsProtected;
  ...
  // The crown-wearing badge is specific to "Seize the crown" levels -- the
  // other objectives have no crown to seize, so they keep only the melt and
  // the status line.
  const seizedCrown = isSolved() && objectiveOf(state.levelIndex) === "reach";
  if (seizedCrown) drawSolvedBadge(elements.solvedBadge, state.levelIndex + 1);
  scheduleBadgeReveal(elements.boardStage, elements.solvedBadge, seizedCrown);
  renderStatus();
...
function renderStatus() {
  ...
  if (isSolved()) {
    ...
    } else if (objectiveOf(state.levelIndex) === "reach") {
      // "Solved!" itself would be redundant here -- the crown badge already
      // says so -- but the last-level notice still needs to go somewhere.
      elements.status.textContent = lastSuffix.trim();
    } else {
      elements.status.textContent = `Solved!${lastSuffix}`;
    }
    elements.status.classList.add("is-won");
  } else {
    const robe = state.hasPotions ? ` · Robe: ${state.current.carried > 0 ? "protected" : "used up"}` : "";
    const prefix = `Level ${state.levelIndex + 1} · Moves: ${moves} · Pushes: ${state.pushes}${robe}`;
    elements.status.textContent = state.commentary && state.message ? `${prefix} · ${state.message}` : prefix;
